...
def build_rgba_overlay(mask: np.ndarray, cmap: dict) -> np.ndarray:
    ...
    Args:
        mask: 3D int array (nx, ny, nz) with class labels
        cmap: dict {label: (r, g, b, a)}

    Returns:
        rgba: (nx, ny, nz, 4) float array in [0, 1]
    """
...
class FETViewer:
    ...
    def _draw_crosshair(self, x: int, y: int):
        ...
        self.ax_img.axvline(x, color="yellow", lw=0.8, alpha=0.6)
        self.ax_img.axhline(y, color="yellow", lw=0.8, alpha=0.6)
        self.fig.canvas.draw_idle()
